...
**Access to the models where the quality gap is largest.** This is the real one. The crossover analysis assumes both options can do the task; a paid frontier model is what makes the API side of that comparison valid for hard reasoning and real coding. **Volatile, dated: As of 2026-09, which models are frontier and what they cost changes on the order of months — check current pricing and model lists rather than any summary, including this one.**
